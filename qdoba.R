...
run.qdoba.sim <- function( service.rate=2, num.mins.open=720) {
	...
	arrival.rate <- 1
        # if the store has been open for 90 mins but not longer than 150 then the 
        # arrival.rate needs to be higher
        # Lunch time
        if(sim.clock >= 90 && sim.clock <= 150){
		arrival.rate <- arrival.rate * 4
		cat("lunch time ",arrival.rate, sim.clock, "\n")
        } # Dinner Time
        else if(sim.clock >= 390 && sim.clock <= 480){
          arrival.rate <- arrival.rate * 2
	  cat("Dinner time ", arrival.rate, sim.clock, "\n")
        }
...
results <- foreach (service.rate=seq(.1,2,.05), .combine=rbind) %dopar% {
    return(run.qdoba.sim(service.rate=service.rate))
}
